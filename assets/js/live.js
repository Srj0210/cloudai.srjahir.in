...
/* ══════════════════════════════════════════════════
   PRIORITY 1 + 7: TTS VIA WORKER PROXY
...
function speakBrowserTTS(text, onDone) {
  ...
  let best = null;
  if (voices.length) {
    for (const name of preferred) { best = voices.find(v => v.name === name); if (best) break; }
...
async function sendToAI(text) {
  ...
  try {
    ...
    // Priority 7: Stream TTS sentence by sentence
    speakStreaming(reply);

  } catch {
    showThinkingDots(false);
    isProcessing = false;
    speakBrowserTTS("Network error. Please check your connection.", () => {
      setStatus("Tap to Talk", "idle");
    });
  }
}
...
function showThinkingDots(show) {
  ...
  if (show && !el) {
    ...
    el.id = "thinking-dots"; el.className = "thinking-dots";
...
if (!SpeechRecognition) {
  ...
  rec.onstart = () => {
    ...
    if (!interimEl) {
      ...
      interimEl.id = "interim-text"; interimEl.className = "interim-text";
  ...
  rec.onend = () => {
    ...
    if (heard) { setStatus(`You: "${heard}"`, "idle"); sendToAI(heard); }
    else { setStatus("Tap to Talk", "idle"); }
  };
  ...
  rec.onerror = e => {
    ...
    if (stopBtn)  stopBtn.style.display  = "none";
    if (interimEl) interimEl.textContent = "";
    const msgs = { "not-allowed": "❌ Mic blocked", "no-speech": "Didn't catch that.", "network": "Network error." };
